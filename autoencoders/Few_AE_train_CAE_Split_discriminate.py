...
for classNum in range(0,10):
    X_train_divided[classNum] = X_train_divided[classNum][:200] / 255
    X_test_divided[classNum] = X_test_divided[classNum][:200] / 255
...
for classNum in range(0,10):
    ...
    checkpoint = ModelCheckpoint("weights/few_weights-ae"+str(classNum)+".hdf5", monitor='val_loss', verbose=1, save_best_only=True, mode='min')
...
for classNum in range(0,10):
    autoencoders[classNum].load_weights("weights/few_weights-ae"+str(classNum)+".hdf5")
